...
def _get_rotation_matrix(rotation_angles):
    """
    Computes a 3x3 rotation matrix from Euler angles (in degrees).
    The rotation is applied in Z-Y-X order.
    """
...
def gen_cube(scale=(1.0, 1.0, 1.0), rotation_angles=(0.0, 0.0, 0.0), center=(0,0,0), color=(0.0, 1.0, 0.0)):
    ...
    rotation_matrix = _get_rotation_matrix(rotation_angles)
...
def gen_rope(num_points=10, distance=0.5, start_pos=(0, 0, 0), rotation_angles=(0.0, 0.0, 0.0), color=(0.0, 1.0, 0.0)):
    """
    Generates vertices and constraints for a rope.

    Args:
        num_points (int): The number of points in the rope.
        distance (float): The distance between adjacent points.
        start_pos (tuple): The position of the first point of the rope.
        rotation_angles (tuple): Euler angles (in degrees) for rotation around the first point.
        color (tuple): The (R, G, B) color of the rope.

    Returns:
        tuple: A tuple containing (vertices, colors, rigid_2p_constraint_indices, rigid_2p_constraint_distance, edge_indices).
    """
    # 1. Generate vertices along the negative Y-axis
    base_vertices = np.zeros((num_points, 3), dtype=np.float32)
    for i in range(1, num_points):
        base_vertices[i] = base_vertices[i-1] + np.array([0, -distance, 0])

    # 2. Get rotation matrix
    rotation_matrix = _get_rotation_matrix(rotation_angles)

    # 3. Apply transformations
    # The rotation is around the first point, which is at the origin of the base vertices.
    # So we just rotate and then translate.
    transformed_vertices = base_vertices @ rotation_matrix.T
    transformed_vertices += np.array(start_pos)

    # 4. Define vertex colors
    vert_colors = np.array([color for _ in range(num_points)], dtype=np.float32)

    # 5. Define edge indices for rendering
    edge_indices = []
    for i in range(num_points - 1):
        edge_indices.extend([i, i + 1])
    edge_indices = np.array(edge_indices, dtype=np.int32)

    # 6. Define distance constraints between adjacent points
    rigid_2p_constraint_indices = edge_indices.copy()
    num_constraints = num_points - 1
    rigid_2p_constraint_distance = np.full(num_constraints, distance, dtype=np.float32)

    return (
        transformed_vertices.astype(np.float32),
        vert_colors,
        rigid_2p_constraint_indices,
        rigid_2p_constraint_distance,
        edge_indices,
    )
...
def check_np_array(np_array):
    print(np_array)
    print("统计信息:")
    print(f"  - 数量 (Count): {np_array.size}")
    print(f"  - 最小值 (Min): {np_array.min()}")
    print(f"  - 最大值 (Max): {np_array.max()}")
    print(f"  - 平均值 (Mean): {np_array.mean()}")
    print(f"  - 是否包含 NaN (Not a Number): {np.isnan(np_array).any()}")
    print(f"  - 是否包含 Inf (Infinity): {np.isinf(np_array).any()}")